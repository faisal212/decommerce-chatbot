...
const AiAgentSinglFileDragAndDrop = styled('div')(({ theme }) => ({
  position: 'relative',
...
interface AiAgentSingleImageDragAndDropProps extends BoxProps {
  onSelectedFile: (file: any) => Promise<void> | void;
...
function formatFileSize(bytes: any) {
  ...
  // If size is less than 1 MB, show in KB, otherwise show in MB
  if (megabytes < 1) {
...
> = ({ ...props }) => {
  ...
  const uploadFile = async (file: File) => {
    try {
      setIsUploading(true);
      await props.onSelectedFile(file);
  ...
  return (
    ...
    >
      <Fade
        in={props.selectedFile && !isUploading ? true : false}
        mountOnEnter
        unmountOnExit
      >
      ...
      {!props.selectedFile && !isUploading && (
        <Stack
      ...
      {props.onDelete && (
        <div
          ...
          style={{
            ...
            width: '30px',
            height: '28px',
            display: props.selectedFile ? 'flex' : 'none',
            flexDirection: 'row',
            alignItems: 'center',
            justifyContent: 'center',
            padding: '6px',
